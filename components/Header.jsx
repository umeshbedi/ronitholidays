...
export default function Header({ Island, }) {
  ...
  useEffect(() => {
    db.collection('blogs').onSnapshot((snap) => {
      const tempBlog = []
      snap.forEach((sndata) => {
        tempBlog.push({ name: sndata.data().name, slug: sndata.data().slug })
      })
  ...
  function RespMenu() {
    return (
      ...
      >
        ...
        <Menu.SubMenu title={<p >Blog{isMobile ? null : <FaAngleDown />}</p>}>
          {
            blogList.map((blogs, key) => (
              <Menu.Item key={key}>
                <Link target='blank' href={blogs.slug}>{blogs.name}</Link>
              </Menu.Item>
            ))
          }
        </Menu.SubMenu>
